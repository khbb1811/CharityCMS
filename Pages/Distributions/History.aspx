<%@ Page Title="التوزيعات" Language="C#" MasterPageFile="~/MasterPages/Admin.master"
    AutoEventWireup="true" CodeBehind="History.aspx.cs"
    Inherits="CharityCMS.Pages.Distributions.History" %>
...
<div class="form-check mb-3">
    <asp:CheckBox ID="chkApplyToAllCity" runat="server"
        CssClass="form-check-input" />
    <label class="form-check-label">
        تطبيق التوزيع على جميع مستفيدي هذه المدينة
    </label>
</div>
                    ...
                    <div class="mb-3">
                        ...
                        <asp:DropDownList ID="ddlDistributionKind" runat="server" CssClass="form-select">
                            <asp:ListItem Text="-- اختر النوع --" Value=""></asp:ListItem>
                            <asp:ListItem Text="عيني" Value="عيني"></asp:ListItem>
                            <asp:ListItem Text="مالي" Value="مالي"></asp:ListItem>
                        </asp:DropDownList>
                    </div>

                    <asp:Panel ID="pnlCashAccount" runat="server" CssClass="mb-3">
                        <label class="form-label">الحساب المالي</label>
                        <asp:DropDownList ID="ddlCashAccount" runat="server" CssClass="form-select"></asp:DropDownList>
                        <asp:Label ID="lblCashBalance" runat="server" CssClass="stock-info"></asp:Label>
                    </asp:Panel>

                    <asp:Panel ID="pnlAmount" runat="server" CssClass="mb-3">
                        <label class="form-label">المبلغ المالي</label>
                        <asp:TextBox ID="txtAmount" runat="server" CssClass="form-control" TextMode="Number"></asp:TextBox>
                        <div class="form-text">يستخدم هذا الحقل للتوزيع المالي فقط</div>
                    </asp:Panel>
                    ...
                    <div class="hint-box">
                        التوزيع المالي يُحفظ بالمبلغ ويُخصم من الحساب المالي المختار.
                        التوزيع العيني يُحفظ بالأصناف والكميات ويُخصم من المخزون العيني.
                    </div>

                    <asp:Panel ID="pnlCart" runat="server" Visible="false" CssClass="mt-4">
                        <hr />
                        <h5 class="mb-3">🛒 سلة التوزيع العيني</h5>
                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive" style="overflow-x: auto;">
                        <asp:GridView ID="gvDist" runat="server"
                            ...
                            AutoGenerateColumns="false"
                            DataKeyNames="DistributionId,DistributionItemId,CashAccountId"
                            EmptyDataText="لا توجد توزيعات"
                            AllowPaging="true"
                            PageSize="20"
                            OnPageIndexChanging="gvDist_PageIndexChanging"
                            ...
                            <Columns>
                                ...
                                <asp:BoundField DataField="DistributionDate" HeaderText="التاريخ" DataFormatString="{0:yyyy-MM-dd}" />
                                <asp:BoundField DataField="DistributionType" HeaderText="نوع التوزيع" />
                                <asp:BoundField DataField="CashAccountNameAr" HeaderText="الحساب المالي" ReadOnly="true" />
                                <asp:BoundField DataField="Amount" HeaderText="المبلغ المالي" DataFormatString="{0:0.##}" />
    ...
    <script type="text/javascript">
        (function () {
            function initBeneficiaryFilter() {
                ...
                if (!ddl.dataset.allOptions) {
                    ...
                    for (var i = 0; i < ddl.options.length; i++) {
                        all.push({ text: ddl.options[i].text, value: ddl.options[i].value });
                    }
            ...
            function toggleDistributionMode() {
                var ddl = document.getElementById('<%= ddlDistributionKind.ClientID %>');
                var pnlAmount = document.getElementById('<%= pnlAmount.ClientID %>');
                var pnlCash = document.getElementById('<%= pnlCashAccount.ClientID %>');
                if (!ddl || !pnlAmount || !pnlCash) return;

                var isCash = ddl.value === 'مالي';
                pnlAmount.style.display = isCash ? 'block' : 'none';
                pnlCash.style.display = isCash ? 'block' : 'none';
            }

            function wireDistributionMode() {
                var ddl = document.getElementById('<%= ddlDistributionKind.ClientID %>');
                if (!ddl) return;
                ddl.removeEventListener('change', toggleDistributionMode);
                ddl.addEventListener('change', toggleDistributionMode);
                toggleDistributionMode();
            }

            function pageInit() {
                initBeneficiaryFilter();
                wireDistributionMode();
            }
